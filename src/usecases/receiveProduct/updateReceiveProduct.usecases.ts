...
import { ValidReceiveProductById } from "./validReceiveProductById.usecase";

export class UpdateReceiveProductUsecases implements IUseCases {
  constructor(private context: PrismaService) {}
  async execute(id: string, item: ReceiveProductItem) {
    await new validProductById(this.context).isValid(item.productId);
    await new ValidReceiveProductById(this.context).isValid(id);
  ...
  private onCalculateNewCostProduct(
    ...
    return calRemainingNewResult / (remainingAmount + newAmount);
  }
}
